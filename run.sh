...
if [[ $step -eq 1 ]] || [[ $step -eq -1 ]]; then
    ...
    steps/make_mfcc.sh --nj $nj --cmd "$train_cmd" $exp_dir/train $exp_dir/make_mfcc/train $exp_dir/mfcc/train
    steps/compute_cmvn_stats.sh $exp_dir/train $exp_dir/make_mfcc/train $exp_dir/mfcc/train

    if [[ -d $exp_dir/test ]]; then
        steps/make_mfcc.sh --nj $nj --cmd "$train_cmd" $exp_dir/test $exp_dir/make_mfcc/test $exp_dir/mfcc/test
        steps/compute_cmvn_stats.sh $exp_dir/test $exp_dir/make_mfcc/test $exp_dir/mfcc/test
    fi
fi
...
if [[ $step -eq 2 ]] || [[ $step -eq -1 ]]; then
    ...
    steps/train_mono.sh --nj $nj --cmd "$train_cmd" $exp_dir/train $exp_dir/lang $out_dir

    utils/mkgraph.sh $exp_dir/lang $out_dir $out_dir/graph

    echo ">>>>> Monophone: decoding"

    steps/decode.sh --nj $nj --cmd "$decode_cmd" --config $PROJ_HOME/conf/decode.conf $out_dir/graph $exp_dir/test $out_dir/decode_test
fi
...
if [[ $step -eq 3 ]] || [[ $step -eq -1 ]]; then
    ...
    steps/align_si.sh --nj $nj  --cmd "$align_cmd" \
        $exp_dir/train $exp_dir/lang $exp_dir/$mono_output_dir $exp_dir/${mono_output_dir}_ali

    echo ">>>>> Deltas: training"

    steps/train_deltas.sh --cmd "$train_cmd" --boost-silence 1.25  $tri1_num_leaves $tri1_num_gauss  \
        $exp_dir/train $exp_dir/lang $exp_dir/${mono_output_dir}_ali $out_dir

    utils/mkgraph.sh $exp_dir/lang $out_dir $out_dir/graph

    echo ">>>>> Deltas: decoding"

    steps/decode.sh --nj $nj --cmd "$decode_cmd" $out_dir/graph $exp_dir/test $out_dir/decode_test
fi
...
if [[ $step -eq 4 ]] || [[ $step -eq -1 ]]; then
    ...
    steps/align_si.sh --nj $nj  --cmd "$align_cmd" --use-graphs true \
        $exp_dir/train $exp_dir/lang $exp_dir/$tri1_output_dir $exp_dir/${tri1_output_dir}_ali

    echo ">>>>> Deltas + Delta-Deltas: training"

    steps/train_deltas.sh --cmd "$train_cmd" --boost-silence 1.25  $tri2_num_leaves $tri2_num_gauss  \
        $exp_dir/train $exp_dir/lang $exp_dir/${tri1_output_dir}_ali $out_dir
...
if [[ $step -eq 5 ]] || [[ $step -eq -1 ]]; then
    ...
    steps/align_si.sh --nj $nj --cmd "$align_cmd" $exp_dir/train $exp_dir/lang \
    ...
    steps/train_lda_mllt.sh --cmd "$train_cmd" $mllt_num_leaves $mllt_num_gauss  \
        $exp_dir/train $exp_dir/lang $exp_dir/${tri2_output_dir}_ali $out_dir
...
if [[ $step -eq 6 ]] || [[ $step -eq -1 ]]; then
    ...
    steps/align_si.sh --nj $nj --cmd "$align_cmd" --use-graphs true $exp_dir/train $exp_dir/lang \
    ...
    steps/train_sat.sh --cmd "$train_cmd" $sat_num_leaves $sat_num_gauss  \
        $exp_dir/train $exp_dir/lang $exp_dir/${mllt_output_dir}_ali $out_dir

    utils/mkgraph.sh $exp_dir/lang  $out_dir $out_dir/graph

    echo ">>>>> LDA-MLLT + SAT: decoding"

    steps/decode_fmllr.sh --nj $nj --cmd "$decode_cmd" $out_dir/graph  $exp_dir/test $out_dir/decode_test
fi
...
if [[ $step -eq 7 ]] || [[ $step -eq -1 ]]; then
    ...
    steps/align_fmllr.sh --nj $nj --cmd "$align_cmd" $exp_dir/train $exp_dir/lang \
        $exp_dir/$sat_output_dir $exp_dir/${sat_output_dir}_ali

    echo ">>>>> SGMM2: training"

    steps/train_ubm.sh --cmd "$train_cmd" $sgmm2_ubm_num_gauss \
        $exp_dir/train $exp_dir/lang $exp_dir/${sat_output_dir}_ali $exp_dir/$sgmm2_ubm_output_dir

    steps/train_sgmm2.sh --cmd "$train_cmd" $sgmm2_num_leaves $sgmm2_num_gauss \
        $exp_dir/train $exp_dir/lang $exp_dir/${sat_output_dir}_ali $exp_dir/$sgmm2_ubm_output_dir/final.ubm $out_dir
    ...
    steps/decode_sgmm2.sh --nj $nj --cmd "$decode_cmd" --transform-dir \
        $exp_dir/$sat_output_dir/decode_test $out_dir/graph $exp_dir/test $out_dir/decode_test
fi
